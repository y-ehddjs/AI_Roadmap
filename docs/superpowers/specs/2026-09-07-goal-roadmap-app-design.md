...
- **백엔드**: 대부분의 CRUD(로드맵/마일스톤/체크인/프로필/리액션 등)는 여전히
  Next.js 클라이언트가 Supabase(Postgres + Auth + Realtime)의 REST(PostgREST)에
  직접 접근한다 — 이 부분엔 커스텀 서버가 없다. 다만 API 키를 클라이언트에 못
  내보내는 AI 호출 지점 세 곳(로드맵 생성, 코칭 지연 감지, 리마인더 발송)만
  별도의 **FastAPI(Python) 백엔드**가 맡는다. 원래는 이 세 곳도 Supabase Edge
  Function(Deno)으로 구현할 계획이었으나, 이 프로젝트가 5주차에 프론트엔드/백엔드를
  각각 도커 컨테이너로 분리하는 과제 요구사항이 있어 그 컨테이너에 실제로 들어갈
  코드가 필요해졌고, 그 자리를 FastAPI가 대신하게 됐다. 이 세 곳 외에 백엔드
  범위를 넓힐 이유는 없다(YAGNI) — 나머지 화면은 그대로 Supabase 직접 접근
  구조를 유지한다.
- **인증**: 이메일/비밀번호 (Supabase Auth). 로그인 안 된 사용자가 보호된 페이지에
  들어오면 `/login`으로, 이미 로그인된 사용자가 `/login`·`/signup`에 들어오면 `/`로
  각각 클라이언트 사이드에서 리다이렉트한다(라우트 가드). FastAPI 백엔드는 위 세
  작업을 실행할 때 Supabase의 service role 키로 접근해 RLS를 우회한다(사용자
  세션과 무관하게 항상 서버 권한으로 동작 — Edge Function이 하던 것과 동일한 방식).
- **알림**: Web Push API (서비스 워커 + VAPID 키 기반 브라우저 푸시, 발송 주체는
  FastAPI 백엔드). 브라우저가 구독을 내부적으로 갱신/만료시키면
  (`pushsubscriptionchange`) 서비스 워커가 새 구독을 열려있는 탭에 전달하고, 그
  탭의(로그인된) 클라이언트가 대신 저장한다 — 열린 탭이 없을 때는 다음 방문
  시까지 갱신이 미뤄지는 MVP 수준의 한계를 감수한다.
- **AI**: Gemini API — (a) 로드맵 자동 생성, (b) 프로액티브 코칭 메시지 생성.
  둘 다 FastAPI 백엔드에서 호출한다.
- **AI 코칭 트리거**: 마일스톤 지연 감지는 FastAPI 백엔드 안에서 APScheduler로
  등록한 주기 작업이 `due_date` 경과 여부를 확인해 트리거한다(Supabase의 cron
  등록 기능 대신, 우리가 띄우는 백엔드 프로세스 안에서 직접 스케줄링). MVP는 이
  지연(delay) 기준 하나만 구현하며, "정체" 같은 별도 판정 기준은 범위 밖(YAGNI)으로
  둔다.
...
10. 로드맵 공개 공유 — 로드맵마다 `is_public` 토글이 있고(기본값 비공개), 켜면
    로그인 없이도 접근 가능한 읽기 전용 링크(`/r/[id]`)가 생긴다. 링크는 전체
    URL을 보여주고 복사 버튼으로 바로 복사할 수 있다. 공개 화면에는
...
12. 리더보드 — 설정에서 "리더보드에 표시"를 켠 사용자들만 골라 계정 통합
    ...
    설정에서 바꿀 수 있다.
13. 커뮤니티 페이지 — 공개(10번) 로드맵을 링크 없이도 둘러볼 수 있는 목록
    화면. 두 가지 정렬을 지원한다: (a) "오늘 하이파이브순" — 오늘 그 로드맵의
    마일스톤들이 받은 하이파이브 개수 합, (b) "추천순" — 전체 기간 누적
    하이파이브 개수 합. 닉네임으로 검색해서 특정 사용자의 공개 로드맵만
    걸러볼 수도 있다. 목록의 각 항목을 누르면 9번(공개 로드맵 보기)으로
    이동한다.
...
11. 커뮤니티 — 공개 로드맵을 오늘 하이파이브순/추천순으로 둘러보거나 닉네임으로
    검색. 항목을 누르면 9번(공개 로드맵 보기)으로 이동.
...
- 비공개 로드맵의 `/r/[id]` 링크로 접근하면(주인이 다시 비공개로 돌렸거나 원래
  ...
  새어나가지 않게 한다
- 공개 화면(`/r/[id]`)은 로드맵/마일스톤을 조회하는 전용 함수를 따로 둬서
  `title`/`due_date`같이 화면에 실제로 그리는 컬럼만 select한다 — 개인용
  화면이 쓰는 `select('*')` 함수를 그대로 재사용하면 화면엔 안 그려도
  로드맵 설명이나 마일스톤 메모가 네트워크 응답에는 그대로 실려서, "공개"로
  표시 안 한 내용까지 개발자 도구로 보이게 된다
- 하이파이브는 같은 사용자가 같은 마일스톤에 중복으로 못 남기게 유니크 제약으로
  막고, 이미 눌렀으면 버튼이 눌린 상태로 보여 다시 누르면 취소된다. 로드맵을
  공개했다가 다시 비공개로 돌려도, 그 로드맵 주인은 자기가 받은 하이파이브를
  계속 조회할 수 있다(공개 여부와 무관한 주인 전용 읽기 권한을 별도로 둠)
